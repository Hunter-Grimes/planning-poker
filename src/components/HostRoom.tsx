...
export function HostRoom({ hostName, roomCode, onClose }: Props) {
  ...
  // Is the active component the last enabled unvoted one?
  const currentComponentIdx = gameState.components.findIndex((s) => s.id === gameState.activeComponentId);
  const hasMoreComponents = gameState.components.some(
    (s, i) => i > currentComponentIdx && s.enabled && s.average === null,
  );
