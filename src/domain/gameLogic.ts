...
// The component that becomes active after the current one is revealed: the next
// enabled, still-unestimated component *after* the active one (forward only — no
// wrap-around). null when the active component is the last votable one. Shared by
// the nextComponent reducer and the host's "Next Component vs Finish" button label
// so the two can't drift.
export function nextComponentAfterActive(state: GameState): Component | null {
  const currentIdx = state.components.findIndex((s) => s.id === state.activeComponentId);
  return state.components.find((s, i) => i > currentIdx && s.enabled && s.average === null) ?? null;
}
...
export function addComponent(state: GameState, component: Component): GameState {
  ...
  const activeComponentId =
    state.activeComponentId === null && state.phase === 'voting'
      ? component.id
...
export function removeComponent(state: GameState, id: string): GameState {
  ...
  const activeComponentId =
    state.activeComponentId === id
      ? nextVotableAfter(components, idx - 1)
      : state.activeComponentId;
  return { ...state, components, activeComponentId };
}
...
export function nextComponent(state: GameState): GameState {
  ...
  if (state.activeComponentId !== null) {
    ...
    components = state.components.map((s) =>
      s.id === state.activeComponentId ? { ...s, average } : s,
    );
  }

  // Forward search is unaffected by recording the active component's average
  // (that one sits at currentIdx, which the search skips), so it's safe to read
  // off the pre-recording state via the shared helper.
  const next = nextComponentAfterActive(state);
